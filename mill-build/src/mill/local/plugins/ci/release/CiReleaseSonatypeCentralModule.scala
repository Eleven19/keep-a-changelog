 ...
 * set the version for example for the user. This should hopefully just be one less thing they need to worry about. The
 * entire goal of this is to make it frictionless for a user to release their project.
 */
trait CiReleaseSonatypeCentralModule extends SonatypeCentralPublishModule {
  override def publishVersion: T[String] = T {
    VcsVersion.vcsState().format(untaggedSuffix = "-SNAPSHOT")
  }

  /**
   * Helper available to users be able to more easily use the new s01 and future hosts for sonatype by just setting
   * this.
   */
...
object SonatypeCentralReleaseModule extends ExternalModule {

  /**
   * This is a replacement for the mill.scalalib.PublishModule/publishAll task that should basically work identically
   * _but_ without requiring the user to pass in anything. It also sets up your gpg stuff and grabs the necessary env
   * variables to publish to sonatype for you.
   */
  ...
  /**
   * Gathers all the CiReleaseModules, which is used to determine what should be released
   */
